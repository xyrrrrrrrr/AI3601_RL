...
class TRPO:
    ...
    def hessian_matrix_vector_product(self, states, old_action_dists, vector):
        ...
        kl_grad_vector_product = torch.dot(kl_grad_vector, vector.detach())
    ...
    def line_search(self, states, actions, advantage, old_log_probs,
        ...
        for i in range(15): 
            ...
            """ YOUR CODE HERE """
            # load new parameter to new actor
            torch.nn.utils.convert_parameters.vector_to_parameters(new_para, new_actor.parameters())
            # compute kl divergence and new objective
            ...
            improve = new_obj - old_obj
            # check if kl is less than constraint and improve is large enough
            if kl < self.kl_constraint and improve > 0:
    ...
    def policy_learn(self, states, actions, old_action_dists, old_log_probs,
        ...
        step_dir = self.conjugate_gradient(grads.data, states, old_action_dists)
        # get hessian vector product
        hd = self.hessian_matrix_vector_product(states, old_action_dists, step_dir)
        # get max coef
        max_coef = torch.sqrt(2 * self.kl_constraint / (torch.dot(step_dir, hd) + 1e-8))
        # get max vec
        max_vec = max_coef * step_dir
        # line search
        self.line_search(states, actions, advantage, old_log_probs, old_action_dists, max_vec)
        """ ------------- Programming 2 ------------- """
    ...
    def update(self, transition_dict):
        ...
        # normalize advantage if needed
        advantage = (advantage - advantage.mean()) / (advantage.std() + 1e-10) 
        # update critic
        critic_loss = torch.nn.functional.mse_loss(values, td_target.detach())
        self.critic_optimizer.zero_grad()
        critic_loss.backward()
        self.critic_optimizer.step()
        # get gradient of loss and hessian vector product of kl divergence
